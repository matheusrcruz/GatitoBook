...
export class ComentariosComponent implements OnInit {
  ...
  gravar(): void{
    const comentario = this.comentarioForm.get('comentario')?.value ?? ''; //safe navigation se não tiver comentari onão salva valor algum
    this.comentarios$ = this.comentariosService.incluiComentario(
      this.id,
      comentario
      ).pipe(switchMap(()=>this.comentariosService.buscaComentario(this.id)),
      tap(()=> {
        this.comentarioForm.reset();  //deprecated
